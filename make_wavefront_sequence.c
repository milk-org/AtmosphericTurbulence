...
{
    if (0 + CLI_checkarg(1, CLIARG_FLOAT) + CLI_checkarg(2, CLIARG_LONG) + CLI_checkarg(3, CLIARG_LONG) == 0)
    {
        // If arguments meet requirements, command is executed
        //
        AtmosphericTurbulence_make_wavefront_sequence(data.cmdargtoken[1].val.numf, data.cmdargtoken[2].val.numl,
...
errno_t AtmosphericTurbulence_make_wavefront_sequence_addCLIcmd()
{
    ...
    RegisterCLIcommand(
        "mkwfseq", __FILE__, AtmosphericTurbulence_make_wavefront_sequence__cli,
        "make atmospheric turbulence wavefront sequence",
        "<wavelength [nm]> <precision 0=single, 1=double> <computation mode>", "mkwfseq 1650.0 1 1",
...
errno_t AtmosphericTurbulence_make_wavefront_sequence(float slambdaum, long WFprecision, int compmode)
{
    ...
    */
    ...
    ATMOSPHERE_MODEL atm = AtmosphereModel_Create_from_CONF(ATMCONFFILE, slambdaum * 1e-6);

    DEBUG_TRACEPOINT("Atmosphere model completed");

    if (0)
    {
        double dens_N2, dens_O2, dens_Ar, dens_H2O, dens_CO2, dens_Ne, dens_He, dens_CH4, dens_Kr, dens_H2, dens_O3,
            dens_N, dens_O, dens_H; // [cm-3]
        double lambda;
        ...
        */
        ...
        denstot = dens_N2 + dens_O2 + dens_Ar + dens_Ne + dens_He + dens_CH4 + dens_Kr + dens_H2 + dens_O3 + dens_N +
                  dens_O + dens_H + dens_CO2;
        ...
        denstot = dens_N2 + dens_O2 + dens_Ar + dens_Ne + dens_He + dens_CH4 + dens_Kr + dens_H2 + dens_O3 + dens_N +
                  dens_O + dens_H + dens_H2O + dens_CO2;
        xN2 = dens_N2 / denstot;
        xO2 = dens_O2 / denstot;
        xAr = dens_Ar / denstot;
        xNe = dens_Ne / denstot;
...

        {
            ...
            for (l = 1e-6; l < 5e-6; l *= 1.0 + 1e-5)
            {
                ...
                fprintf(fpRlambda, "%.16f %.16f %.16f %.16f\n", l, riav0.rindex, riav1.rindex, riav2.rindex);
            }
            fclose(fpRlambda);
        }
    ...
    */
    ...
    {
        create_3Dimage_ID("outarraypha", naxesout[0], naxesout[1], naxesout[2], &IDout_array_pha);
        create_3Dimage_ID("outarrayamp", naxesout[0], naxesout[1], naxesout[2], &IDout_array_amp);
        create_3Dimage_ID("outsarraypha", naxesout[0], naxesout[1], naxesout[2], &IDout_sarray_pha);
        create_3Dimage_ID("outsarrayamp", naxesout[0], naxesout[1], naxesout[2], &IDout_sarray_amp);
    ...
    {
        create_3Dimage_ID_double("outarraypha", naxesout[0], naxesout[1], naxesout[2], &IDout_array_pha);
        create_3Dimage_ID_double("outarrayamp", naxesout[0], naxesout[1], naxesout[2], &IDout_array_amp);
        create_3Dimage_ID_double("outsarraypha", naxesout[0], naxesout[1], naxesout[2], &IDout_sarray_pha);
        create_3Dimage_ID_double("outsarrayamp", naxesout[0], naxesout[1], naxesout[2], &IDout_sarray_amp);
        for (uint64_t ii = 0; ii < naxesout[0] * naxesout[1] * naxesout[2]; ii++)
        {
...

    {
        ...
        for (long layer = 0; layer < NBLAYERS; layer++)
        {
            printf("Turbulence layer %ld : alt = %f m   CN2 = %f   V = %f m/s   Angle = %f rad   outerscale = %f m    "
                   "innerscale = %f m  sigmaWind = %f m/s  Lwind = %f m\n",
                   layer, LAYER_ALT[layer], LAYER_CN2[layer], LAYER_SPD[layer], LAYER_DIR[layer],
                   LAYER_OUTERSCALE[layer], LAYER_INNERSCALE[layer], LAYER_SIGMAWSPEED[layer], LAYER_LWIND[layer]);
        }
    }
    ...
    if (WFprecision == 0) // single precision
    {
        ...
        create_2Dimage_ID("peakphare_bin", xsizepeakpha, ysizepeakpha, &IDpeakpha_re_bin);
        create_2Dimage_ID("peakphaim_bin", xsizepeakpha, ysizepeakpha, &IDpeakpha_im_bin);
        create_2Dimage_ID("peakpha_bin", xsizepeakpha, ysizepeakpha, &IDpeakpha_bin);
        create_2Dimage_ID("peakpha_bin_ch", xsizepeakpha, ysizepeakpha, &IDpeakpha_bin_ch);
    }
    else
    {
        ...
        xsizepeakpha = (uint32_t)(naxesout[0] / 20);
        ysizepeakpha = (uint32_t)(naxesout[1] / 20);
        create_2Dimage_ID_double("peakphare_bin", xsizepeakpha, ysizepeakpha, &IDpeakpha_re_bin);
        create_2Dimage_ID_double("peakphaim_bin", xsizepeakpha, ysizepeakpha, &IDpeakpha_im_bin);
        create_2Dimage_ID_double("peakpha_bin", xsizepeakpha, ysizepeakpha, &IDpeakpha_bin);
        create_2Dimage_ID_double("peakpha_bin_ch", xsizepeakpha, ysizepeakpha, &IDpeakpha_bin_ch);
    }
    ...
    long *super_layer_index;
    {
        ...
        while (OK == 0)
        {
            ...
            printf("minimumum distance between layers: %.2f m  (%ld %ld)   (FresnelPropBin = %.2f)\n", minaltd,
                   layerindex, layerindex + 1, atmturbconf.FresnelPropBin);
            ...
            else
            {
                ...
                SLAYER_ALT[layerindex] = (SLAYER_CN2[layerindex] * SLAYER_ALT[layerindex] +
        ...
        for (long layer = 0; layer < NBLAYERS; layer++)
        {
            ...
            for (h = atm.SiteAlt; h < LAYER_ALT[layer]; h += 1.0)
            {
                ...
                tmpf = sin(atmturbconf.zenithangle) /
                       sqrt(Rindex * Rindex - sin(atmturbconf.zenithangle) * sin(atmturbconf.zenithangle));
                tmpf -= sin(atmturbconf.zenithangle) /
                        sqrt(1.0 - sin(atmturbconf.zenithangle) * sin(atmturbconf.zenithangle));
                Roffset += tmpf;
            ...
            xpos[layer] += atmturbconf.sourceXpos * LAYER_ALT[layer] / (atmturbconf.PupilScale / pfactor);
            ypos[layer] += atmturbconf.sourceYpos * LAYER_ALT[layer] / (atmturbconf.PupilScale / pfactor);

            xpos0[layer] =
                atmturbconf.sourceXpos * LAYER_ALT[layer] / (atmturbconf.PupilScale / pfactor); // for realtime mode
            ypos0[layer] = atmturbconf.sourceYpos * LAYER_ALT[layer] / (atmturbconf.PupilScale / pfactor);

            vxpix[layer] = LAYER_SPD[layer] * cos(LAYER_DIR[layer]) /
                           (atmturbconf.PupilScale / pfactor); /* pixel coordinate speed, in pixel per sec, x axis */
            vypix[layer] = LAYER_SPD[layer] * sin(LAYER_DIR[layer]) /
                           (atmturbconf.PupilScale / pfactor); /* pixel coordinate speed, in pixel per sec, y axis */
    ...
    for (long i = 0; i < NBMASTERS; i++)
    {
        ...
        if (IDt == -1)
        {
            printf("CREATING %s   (%f - %f)\n", fnamemasterwf, LAYER_OUTERSCALE[i] / atmturbconf.PupilScale,
                   LAYER_INNERSCALE[i] / atmturbconf.PupilScale);
            AtmosphericTurbulence_make_master_turbulence_screen(
                imnamemasterwf, "tursctmp", atmturbconf.MasterSize, LAYER_OUTERSCALE[i] / atmturbconf.PupilScale,
                LAYER_INNERSCALE[i] / atmturbconf.PupilScale, WFprecision);
     ...
     */
    {
        ...
        double normcoeff = pow(
            r0 / (atmturbconf.lambda / (atmturbconf.seeing / 3600.0 / 180.0 * PI) / atmturbconf.PupilScale * pfactor),
            5.0 / 6.0);
        if (WFprecision == 0)
        {
            for (long k = 0; k < NBMASTERS; k++)
    ...
    else
    {
        ...
        if (atmturbconf.flag_SWF_make == 1)
        {
            if ((sarray_double = (complex_double *)malloc(NBFRAMES * naxes[0] * naxes[1] * sizeof(complex_double))) ==
                NULL)
            {
    ...
    if (atmturbconf.flag_SkipExisting == 1)
    {
        ...
        while (OK == 1)
        {
            ...
            WRITE_FILENAME(fnameswf, "%s%08ld.%09ld.pha.fits", atmturbconf.SWFfileprefix, start_cubeindex,
     ...
     */
    {
        ...
        for (long layer = 0; layer < NBLAYERS; layer++)
        {
            ...
            if (ID == -1)
            {
                ...
                make_AtmosphericTurbulence_vonKarmanWind(vKwindsize, WSPEEDpixscale, LAYER_SIGMAWSPEED[layer],
                                                         LAYER_LWIND[layer], WSPEEDsize, imnamewspeed);
                ...
                {
                    fprintf(fpwsp, "%6ld   %20.16f   %20.16f  %.16g  %.16g  %.16g\n", ii, WSPEEDpixscale * ii,
                            WSPEEDpixscale * ii * LAYER_SPD[layer], data.image[ID].array.F[ii],
                            data.image[ID].array.F[vKwindsize + ii], data.image[ID].array.F[2 * vKwindsize + ii]);
                }
                fclose(fpwsp);
            }
        }
    ...
    for (long cubeindex = start_cubeindex; cubeindex < atmturbconf.NBTimeCube; cubeindex++)
    {
        for (long frame = 0; frame < NBFRAMES; frame++)
        {
            ...
            for (long layer = NBLAYERS - 1; layer != -1; layer--)
            {
                if (atmturbconf.flag_RealTime == 0)
                {
                    ...
                    printf("\rLayer %2ld/%2ld, Frame %4ld/%4ld, File %6ld/%6ld  [TIME = %10.4f s]  ", layer, NBLAYERS,
                           frame, NBFRAMES, cubeindex, atmturbconf.NBTimeCube,
                           (cubeindex * NBFRAMES + frame) * atmturbconf.TimeStep);
                }
                else
                {
                    printf("\rLayer %2ld/%2ld, Frame %4ld/%4ld, File %6ld/%6ld  [PHYSICAL TIME = %.9lf s]  ", layer,
                           NBLAYERS, frame, NBFRAMES, cubeindex, atmturbconf.NBTimeCube, tnowdouble);
                }
                fflush(stdout);
                    ...
                    if (super_layer_index[layer + 1] != super_layer_index[layer])
                    {
                        if (atmturbconf.flag_FresnelProp == 1)
                        {
                            Fresnel_propagate_wavefront(
                                "array2", "array2p", atmturbconf.PupilScale / pfactor,
                                (SLAYER_ALT[super_layer_index[layer + 1]] - SLAYER_ALT[super_layer_index[layer]]) /
                        ...
                        if (atmturbconf.flag_SWF_make == 1)
                        {
                            if (atmturbconf.flag_FresnelProp == 1)
                            {
                                ...
                                Fresnel_propagate_wavefront(
                                    "sarray2", "sarray2p", atmturbconf.PupilScale / pfactor,
                                    (SLAYER_ALT[super_layer_index[layer + 1]] - SLAYER_ALT[super_layer_index[layer]]) /
                                        cos(atmturbconf.zenithangle),
                                    SLAMBDA);
                        ...
                        for (uint32_t ii = 0; ii < naxes[0]; ii++)
                            for (uint32_t jj = 0; jj < naxes[1]; jj++)
                            {
                                ...
                                double value = (1.0 - iifrac) * (1.0 - jjfrac) *
                                               data.image[ID_TML[layer]].array.F[jjm * naxes_MASTER[0] + iim];
                                value += (1.0 - iifrac) *
                                         (jjfrac)*data.image[ID_TML[layer]].array.F[jjm1 * naxes_MASTER[0] + iim];
                                value += (iifrac) *
                                         (jjfrac)*data.image[ID_TML[layer]].array.F[jjm1 * naxes_MASTER[0] + iim1];
                                value += (iifrac) * (1.0 - jjfrac) *
                                         data.image[ID_TML[layer]].array.F[jjm * naxes_MASTER[0] + iim1];
                        ...
                        for (uint32_t ii = 0; ii < naxes[0]; ii++)
                            for (uint32_t jj = 0; jj < naxes[1]; jj++)
                            {
                                ...
                                */
                                ...
                                a12 =
                                    -.5 * p00 + 1.25 * p01 - p02 + .25 * p03 + .5 * p20 - 1.25 * p21 + p22 - .25 * p23;
                                a13 = .25 * p00 - .75 * p01 + .75 * p02 - .25 * p03 - .25 * p20 + .75 * p21 -
                                      .75 * p22 + .25 * p23;
                                a20 = p01 - 2.5 * p11 + 2 * p21 - .5 * p31;
                                a21 =
                                    -.5 * p00 + .5 * p02 + 1.25 * p10 - 1.25 * p12 - p20 + p22 + .25 * p30 - .25 * p32;
                                ...
                                a31 = .25 * p00 - .25 * p02 - .75 * p10 + .75 * p12 + .75 * p20 - .75 * p22 -
                                      .25 * p30 + .25 * p32;
                                a32 = -.5 * p00 + 1.25 * p01 - p02 + .25 * p03 + 1.5 * p10 - 3.75 * p11 + 3 * p12 -
                                      .75 * p13 - 1.5 * p20 + 3.75 * p21 - 3 * p22 + .75 * p23 + .5 * p30 - 1.25 * p31 +
                                      p32 - .25 * p33;
                                a33 = .25 * p00 - .75 * p01 + .75 * p02 - .25 * p03 - .75 * p10 + 2.25 * p11 -
                                      2.25 * p12 + .75 * p13 + .75 * p20 - 2.25 * p21 + 2.25 * p22 - .75 * p23 -
                    ...
                    if (BICUBIC == 0)
                    {
                        ...
                        for (uint32_t ii = 0; ii < naxes[0]; ii++)
                            for (uint32_t jj = 0; jj < naxes[1]; jj++)
                            {
                                ...
                                double value = (1.0 - iifrac) * (1.0 - jjfrac) *
                                               data.image[ID_TML[layer]].array.D[jjm * naxes_MASTER[0] + iim];
                                value += (1.0 - iifrac) *
                                         (jjfrac)*data.image[ID_TML[layer]].array.D[jjm1 * naxes_MASTER[0] + iim];
                                value += (iifrac) *
                                         (jjfrac)*data.image[ID_TML[layer]].array.D[jjm1 * naxes_MASTER[0] + iim1];
                                value += (iifrac) * (1.0 - jjfrac) *
                                         data.image[ID_TML[layer]].array.D[jjm * naxes_MASTER[0] + iim1];
                                ...
                                for (uint32_t jj = 0; jj < naxes[1]; jj++)
                                {
                                    ...
                                    */
                                    ...
                                    a22 = p00 - 2.5 * p01 + 2 * p02 - .5 * p03 - 2.5 * p10 + 6.25 * p11 - 5 * p12 +
                                          ...
                                          .25 * p33;
                                    a23 = -.5 * p00 + 1.5 * p01 - 1.5 * p02 + .5 * p03 + 1.25 * p10 - 3.75 * p11 +
                                          3.75 * p12 - 1.25 * p13 - p20 + 3 * p21 - 3 * p22 + p23 + .25 * p30 -
                                          .75 * p31 + .75 * p32 - .25 * p33;
                                    a30 = -.5 * p01 + 1.5 * p11 - 1.5 * p21 + .5 * p31;
                                    a31 = .25 * p00 - .25 * p02 - .75 * p10 + .75 * p12 + .75 * p20 - .75 * p22 -
                                          .25 * p30 + .25 * p32;
                                    a32 = -.5 * p00 + 1.25 * p01 - p02 + .25 * p03 + 1.5 * p10 - 3.75 * p11 + 3 * p12 -
                                          .75 * p13 - 1.5 * p20 + 3.75 * p21 - 3 * p22 + .75 * p23 + .5 * p30 -
                                          1.25 * p31 + p32 - .25 * p33;
                                    a33 = .25 * p00 - .75 * p01 + .75 * p02 - .25 * p03 - .75 * p10 + 2.25 * p11 -
                                          2.25 * p12 + .75 * p13 + .75 * p20 - 2.25 * p21 + 2.25 * p22 - .75 * p23 -
                                          .25 * p30 + .75 * p31 - .75 * p32 + .25 * p33;
            ...
            if (WFprecision == 0)
            {
                for (uint32_t ii = 0; ii < naxesout[0]; ii++)
                    for (uint32_t jj = 0; jj < naxesout[1]; jj++)
                    {
                        ...
                        data.image[IDout_array_pha].array.F[frame * naxesout[0] * naxesout[1] + jj * naxesout[0] + ii] =
            ...
            else
            {
                for (uint32_t ii = 0; ii < naxesout[0]; ii++)
                    for (uint32_t jj = 0; jj < naxesout[1]; jj++)
                    {
                        long ii1 = ii + (naxes[0] - naxesout[0]) / 2;
                        long jj1 = jj + (naxes[1] - naxesout[1]) / 2;
                        data.image[IDout_array_pha].array.D[frame * naxesout[0] * naxesout[1] + jj * naxesout[0] + ii] =
                    ...
                    for (uint32_t ii = 0; ii < naxesout[0]; ii++)
                        for (uint32_t jj = 0; jj < naxesout[1]; jj++)
                        {
                            ...
                            data.image[IDout_array_amp]
                                .array.F[frame * naxesout[0] * naxesout[1] + jj * naxesout[0] + ii] =
                                sqrt(re * re + im * im);
                            ...
                            data.image[IDout_array_pha]
                                ...
                                pha +
                                2.0 * M_PI *
                                    ((long)(data.image[IDout_array_pha].array.F[frame * naxesout[0] * naxesout[1] +
                                                                                jj * naxesout[0] + ii] /
                                                2.0 / M_PI +
                                            1000.5) -
                                     1000.0);
            ...
            else
            {
                for (uint32_t ii = 0; ii < naxesout[0]; ii++)
                    for (uint32_t jj = 0; jj < naxesout[1]; jj++)
                    {
                        ...
                        double re = array_double[frame * naxesout[0] * naxesout[1] + jj * naxesout[0] + ii].re;
                        double im = array_double[frame * naxesout[0] * naxesout[1] + jj * naxesout[0] + ii].im;
                        data.image[IDout_array_amp].array.D[frame * naxesout[0] * naxesout[1] + jj * naxesout[0] + ii] =
                            sqrt(re * re + im * im);
                        ...
                        data.image[IDout_array_pha].array.D[frame * naxesout[0] * naxesout[1] + jj * naxesout[0] + ii] =
                            ...
                            2.0 * M_PI *
                                ((long)(data.image[IDout_array_pha]
                                                .array.D[frame * naxesout[0] * naxesout[1] + jj * naxesout[0] + ii] /
                                            2.0 / M_PI +
                                        1000.5) -
                                 1000.0);
                    }
            ...
            if (atmturbconf.flag_SWF_SHMoutput == 1)
            {
                    ...
                    if (WFprecision == 0)
                    {
                        for (uint64_t ii = 0; ii < naxesout[0] * naxesout[1]; ii++)
                        {
                            data.image[IDshmpha].array.F[ii] =
                                data.image[ID_array1].array.F[frame * naxesout[0] * naxesout[1] + ii];
                        }
                    }
                    else
                    {
                        for (uint64_t ii = 0; ii < naxesout[0] * naxesout[1]; ii++)
                        {
                            data.image[IDshmpha].array.D[ii] =
                                data.image[ID_array1].array.D[frame * naxesout[0] * naxesout[1] + ii];
                        }
                    }
                }
                else
                {
                    if (WFprecision == 0)
                    {
                        ...
                        {
                            data.image[IDshmpha].array.F[ii] =
                                data.image[IDout_array_pha].array.F[frame * naxesout[0] * naxesout[1] + ii];
                            data.image[IDshmamp].array.F[ii] =
                                data.image[IDout_array_amp].array.F[frame * naxesout[0] * naxesout[1] + ii];
                        }
                    }
                    else
                    {
                        ...
                        {
                            data.image[IDshmpha].array.D[ii] =
                                data.image[IDout_array_pha].array.D[frame * naxesout[0] * naxesout[1] + ii];
                            data.image[IDshmamp].array.D[ii] =
                                data.image[IDout_array_amp].array.D[frame * naxesout[0] * naxesout[1] + ii];
                        }
                    }
                }
            }
            ...
            if (atmturbconf.flag_SWF_make == 1)
            {
                    ...
                    for (uint32_t ii = 0; ii < naxesout[0]; ii++)
                        for (uint32_t jj = 0; jj < naxesout[1]; jj++)
                        {
                            ...
                            data.image[IDout_sarray_pha]
                                .array.F[frame * naxesout[0] * naxesout[1] + jj * naxesout[0] + ii] =
                                data.image[ID_sarray1].array.F[jj1 * naxes[0] + ii1];
                        }
                }
                else
                {
                    for (uint32_t ii = 0; ii < naxesout[0]; ii++)
                        for (uint32_t jj = 0; jj < naxesout[1]; jj++)
                        {
                            ...
                            data.image[IDout_sarray_pha]
                                .array.D[frame * naxesout[0] * naxesout[1] + jj * naxesout[0] + ii] =
                                data.image[ID_sarray1].array.D[jj1 * naxes[0] + ii1];
                        }
                }
                ...
                if (atmturbconf.flag_WFampl == 1)
                {
                        ...
                        for (uint32_t ii = 0; ii < naxesout[0]; ii++)
                            for (uint32_t jj = 0; jj < naxesout[1]; jj++)
                            {
                                ...
                                data.image[IDout_sarray_amp]
                                    ...
                                    sqrt(re * re + im * im);
                                float pha = atan2(im, re);
                                data.image[IDpeakpha_re].array.F[jj * naxesout[0] + ii] =
                                    cos(data.image[IDout_sarray_pha]
                                            .array.F[frame * naxesout[0] * naxesout[1] + jj * naxesout[0] + ii] -
                                        pha);
                                data.image[IDpeakpha_im].array.F[jj * naxesout[0] + ii] =
                                    sin(data.image[IDout_sarray_pha]
                                            .array.F[frame * naxesout[0] * naxesout[1] + jj * naxesout[0] + ii] -
                                        pha);
                                data.image[IDout_sarray_pha]
                                    .array.F[frame * naxesout[0] * naxesout[1] + jj * naxesout[0] + ii] = pha;
                                ...
                                {
                                    data.image[IDpeakpha_re_bin].array.F[jj2 * xsizepeakpha + ii2] +=
                                        cos(data.image[ID_sarray1].array.F[jj1 * naxes[0] + ii1] - pha);
                                    data.image[IDpeakpha_im_bin].array.F[jj2 * xsizepeakpha + ii2] +=
                                        sin(data.image[ID_sarray1].array.F[jj1 * naxes[0] + ii1] - pha);
                                }
                            }
                    }
                    else
                    {
                        for (uint32_t ii = 0; ii < naxesout[0]; ii++)
                            for (uint32_t jj = 0; jj < naxesout[1]; jj++)
                            {
                                ...
                                double re = sarray_double[frame * naxesout[0] * naxesout[1] + jj * naxesout[0] + ii].re;
                                double im = sarray_double[frame * naxesout[0] * naxesout[1] + jj * naxesout[0] + ii].im;
                                data.image[IDout_sarray_amp]
                                    .array.D[frame * naxesout[0] * naxesout[1] + jj * naxesout[0] + ii] =
                                    sqrt(re * re + im * im);
                                double pha = atan2(im, re);
                                data.image[IDpeakpha_re].array.D[jj * naxesout[0] + ii] =
                                    cos(data.image[IDout_sarray_pha]
                                            .array.D[frame * naxesout[0] * naxesout[1] + jj * naxesout[0] + ii] -
                                        pha);
                                data.image[IDpeakpha_im].array.D[jj * naxesout[0] + ii] =
                                    sin(data.image[IDout_sarray_pha]
                                            .array.D[frame * naxesout[0] * naxesout[1] + jj * naxesout[0] + ii] -
                                        pha);
                                data.image[IDout_sarray_pha]
                                    .array.D[frame * naxesout[0] * naxesout[1] + jj * naxesout[0] + ii] = pha;
                                long ii2 = (long)(1.0 * ii / naxesout[0] * xsizepeakpha);
                                long jj2 = (long)(1.0 * jj / naxesout[1] * ysizepeakpha);

                                if ((ii2 < xsizepeakpha) && (jj2 < ysizepeakpha))
                                {
                                    data.image[IDpeakpha_re_bin].array.D[jj2 * xsizepeakpha + ii2] +=
                                        cos(data.image[ID_sarray1].array.D[jj1 * naxes[0] + ii1] - pha);
                                    data.image[IDpeakpha_im_bin].array.D[jj2 * xsizepeakpha + ii2] +=
                                        sin(data.image[ID_sarray1].array.D[jj1 * naxes[0] + ii1] - pha);
                                }
                            }
                    }
                    ...
                    if (WFprecision == 0)
                    {
                        for (uint64_t ii2 = 0; ii2 < xsizepeakpha * ysizepeakpha; ii2++)
                        {
                            data.image[IDpeakpha_bin].array.F[ii2] = atan2(data.image[IDpeakpha_im_bin].array.F[ii2],
                    ...
                    else
                    {
                        for (uint64_t ii2 = 0; ii2 < xsizepeakpha * ysizepeakpha; ii2++)
                        {
                            data.image[IDpeakpha_bin].array.D[ii2] = atan2(data.image[IDpeakpha_im_bin].array.D[ii2],
                    ...
                    if (WFprecision == 0)
                    {
                        for (uint32_t ii = 0; ii < naxesout[0]; ii++)
                            for (uint32_t jj = 0; jj < naxesout[1]; jj++)
                            {
                                ...
                                float pha = data.image[IDout_sarray_pha]
                                                .array.F[frame * naxesout[0] * naxesout[1] + jj * naxesout[0] + ii];
                                data.image[IDout_sarray_pha]
                                    .array.F[frame * naxesout[0] * naxesout[1] + jj * naxesout[0] + ii] =
                                    pha +
                                    2.0 * M_PI *
                                        ((long)((data.image[ID_sarray1].array.F[jj1 * naxes[0] + ii1] - pha - peakpha) /
                                                    2.0 / M_PI +
                                                1000.5) -
                                         1000.0);
                            }
                    ...
                    else
                    {
                        for (uint32_t ii = 0; ii < naxesout[0]; ii++)
                            for (uint32_t jj = 0; jj < naxesout[1]; jj++)
                            {
                                ...
                                double pha = data.image[IDout_sarray_pha]
                                                 .array.D[frame * naxesout[0] * naxesout[1] + jj * naxesout[0] + ii];
                                data.image[IDout_sarray_pha]
                                    .array.D[frame * naxesout[0] * naxesout[1] + jj * naxesout[0] + ii] =
                                    pha +
                                    2.0 * M_PI *
                                        ((long)((data.image[ID_sarray1].array.D[jj1 * naxes[0] + ii1] - pha - peakpha) /
                                                    2.0 / M_PI +
                                                1000.5) -
                                         1000.0);
                            }
            ...
            if ((atmturbconf.flag_SHMoutput == 1) && (atmturbconf.flag_SWF_make == 1))
            {
                    ...
                    if (WFprecision == 0)
                    {
                        for (uint64_t ii = 0; ii < naxesout[0] * naxesout[1]; ii++)
                        {
                            data.image[IDshmspha].array.F[ii] =
                                data.image[ID_sarray1].array.F[frame * naxesout[0] * naxesout[1] + ii] * coeff;
                        }
                    }
                    else
                    {
                        for (uint64_t ii = 0; ii < naxesout[0] * naxesout[1]; ii++)
                        {
                            data.image[IDshmspha].array.D[ii] =
                                data.image[ID_sarray1].array.D[frame * naxesout[0] * naxesout[1] + ii] * coeff;
                        }
                    }
                ...
                else
                {
                    ...
                    if (WFprecision == 0)
                    {
                        ...
                        {
                            data.image[IDshmspha].array.F[ii] =
                                data.image[IDout_sarray_pha].array.F[frame * naxesout[0] * naxesout[1] + ii] * coeff;
                            data.image[IDshmsamp].array.F[ii] =
                                data.image[IDout_sarray_amp].array.F[frame * naxesout[0] * naxesout[1] + ii];
                        }
                    }
                    else
                    {
                        ...
                        {
                            data.image[IDshmspha].array.D[ii] =
                                data.image[IDout_sarray_pha].array.D[frame * naxesout[0] * naxesout[1] + ii] * coeff;
                            data.image[IDshmsamp].array.D[ii] =
                                data.image[IDout_sarray_amp].array.D[frame * naxesout[0] * naxesout[1] + ii];
                        }
                    }
                    data.image[IDshmspha].md[0].cnt0++;
        ...
        if (atmturbconf.flag_WFoutput == 1) // WRITE REFERENCE LAMBDA
        {
            ...
            WRITE_FILENAME(fnameoutpha, "%s%08ld.%09ld.pha.fits", atmturbconf.WFfileprefix, cubeindex,
            ...
            if (atmturbconf.flag_WFampl == 1)
            {
                ...
                WRITE_FILENAME(fnameoutamp, "%s%08ld.%09ld.amp.fits", atmturbconf.WFfileprefix, cubeindex,
                               (long)(1.0e12 * SLAMBDA + 0.5));
        ...
        if ((atmturbconf.flag_SWF_make == 1) && (atmturbconf.flag_SWF_Wite2Disk == 1)) // WRITE SCIENCE LAMBDA
        {
            printf("WRITING WAVEFRONT FILE ...");
            fflush(stdout);

            char fnameoutpha[STRINGMAXLEN_FILENAME];
            WRITE_FILENAME(fnameoutpha, "%s%08ld.%09ld.pha.fits", atmturbconf.SWFfileprefix, cubeindex,
                           (long)(1.0e12 * SLAMBDA + 0.5));
            ...
            {
                char fnameoutamp[STRINGMAXLEN_FILENAME];
                WRITE_FILENAME(fnameoutamp, "%s%08ld.%09ld.amp.fits", atmturbconf.SWFfileprefix, cubeindex,
                               (long)(1.0e12 * SLAMBDA + 0.5));
