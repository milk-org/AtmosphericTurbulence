...
errno_t AtmosphericTurbulence_ReadConf(const char *restrict fnameconf, ATMTURBCONF *atmturbconf)
{
    ...
    atmturbconf->lambda = read_config_parameter_float(fnameconf, "TURBULENCE_REF_WAVEL") * 0.000001;
    atmturbconf->seeing = read_config_parameter_float(fnameconf, "TURBULENCE_SEEING");
    read_config_parameter(fnameconf, "TURBULENCE_PROF_FILE", atmturbconf->turbulenceprof_fname);
