...
errno_t AtmosphericTurbulence_makeHV_CN2prof(
    double wspeed, double r0, double sitealt, long NBlayer, const char *outfile)
{
    FILE   *fp;
    double  h;
    double  CN2;
    ...
    for (iter = 0; iter < 30; iter++)
    {
        ...
        r0val = 1.0 /
                pow(0.423 * pow(2.0 * M_PI / lambda, 2.0) * CN2sum, 3.0 / 5.0);
    ...
    for (k = 0; k < NBlayer; k++)
    {
        ...
        layerarray_Wspeed[k] =
            wspeed * (0.3 + 0.8 * sqrt(1.0 * k / (1.0 + NBlayer)));
        ...
        fprintf(fp,
                ...
                layerarray_h[k],
                layerarray_CN2frac[k],
                layerarray_Wspeed[k],
                2.0 * M_PI * k / (1.0 + NBlayer),
                L0,
                l0,
                layerarray_sigmaWindSpeed[k],
                layerarray_Lwind[k]);
    }
    fclose(fp);
