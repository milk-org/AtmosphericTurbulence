...
errno_t AtmosphericTurbulence_makeHV_CN2prof(double wspeed, double r0, double sitealt, long NBlayer,
                                             const char *outfile)
{
    FILE *fp;
    double h;
    ...
    for (iter = 0; iter < 30; iter++)
    {
        ...
        {
            CN2 = 5.94e-53 * pow(wspeed / 27.0, 2.0) * pow(h, 10.0) * exp(-h / 1000.0) + 2.7e-16 * exp(-h / 1500.0) +
                  A * exp(-(h - sitealt) / 100.0);
            CN2sum += CN2 / hstep;
        }
    ...
    fp = fopen(outfile, "w");
    fprintf(fp, "# altitude(m)   relativeCN2     speed(m/s)   direction(rad) outerscale[m] innerscale[m] sigmaWsp[m/s] "
                "Lwind[m]\n");
    fprintf(fp, "\n");
    for (k = 0; k < NBlayer; k++)
    {
        ...
        fprintf(fp, "%12f  %12f  %12f  %12f  %12f  %12f  %12f  %12f\n", layerarray_h[k], layerarray_CN2frac[k],
                layerarray_Wspeed[k], 2.0 * M_PI * k / (1.0 + NBlayer), L0, l0, layerarray_sigmaWindSpeed[k],
                layerarray_Lwind[k]);
    }
    fclose(fp);
